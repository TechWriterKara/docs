...
Follow this tutorial to setup a test project in Katalon Studio. We assume you are familiar with general principles of automated testing and have minimal knowledge of the Katalon Studio IDE.

Internet Explorer Configurations
--------------------------------

Skip this section if you do not wish to run your automated tests on Internet Explorer. Otherwise, use the following setup:
...
– Set the **Zoom** level on IE to **100%** so native mouse events can be identified correctly.
...
– For IE 11, set a registry entry on the target computer so the Katalon driver can maintain a connection to the IE instances it creates.

1.Type '**_regedit_**' into **Command Prompt** to open **Registry Editor**.

2\. Locate the **FEATURE_BFCACHE** subkey or create the **FEATURE_BFCACHE** subkey if it is not there:
...
Use this configuration to decide which port to use to send data between Katalon Utility and Katalon Studio.
...
2\. Open preferences for Katalon Utility in Katalon Studio by accessing **Window > Preferences > Katalon > Utility Addon**. Enter the same port number to communicate with Katalon Utility.
...
If your machine uses a restricted network that only allows Internet connection through a proxy server, proxy settings can be configured from **Preferences > Proxy**. The settings affect both WebUI and WebService test execution. (Contact your network administrator for any necessary settings information.)
...
Users may find their computers operating in a network that requires certification to access the Internet. If so, Katalon Studio supports the capability to bypass certificate validation so users with restricted network policies can work with Katalon Studio as usual. This setting can be found in **Project > Settings > Network** and affects both WebUI and WebService testing.
